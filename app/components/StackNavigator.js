        ...
        GroupCall("GID", groupIdList)
            .then((result) => {
                if (result === 'No GID token found') {
                    ...
                    setCurrentGroupId(result)
                }
            })
    }, [groupIdList]);

    const getGroupInfo = async (groupId) => {
        const api = `/home/group/${groupId}`;
        setCurrentGroupId(groupId);
        try {
            const data = await call(api, true, "GET");
            await Keychain.setInternetCredentials("GroupInfo", data.result.groupUserRole, data.result.groupId.toString());
        } catch (error) {
            console.error("Error fetching group info or setting keychain:", error);
        }
    };

    const onPressHandler = (groupId, groupName) => {
        getGroupInfo(groupId, null)
        groupNameHandler(groupName)
    }
              ...
              <Text style={styles.logoutBtnText}>로그아웃 하기</Text>
            </TouchableOpacity> */}
                    {
                        groupList.map((group, idx) => (
                            <View key={idx}>
                                <TouchableOpacity onPress={() => onPressHandler(group.group_id, group.name)}>
                                    <Text style={Number(currentGroupId) === group.group_id ? styles.overlayGroupNameMain : styles.overlayGroupName}>{group.name}</Text>
                                </TouchableOpacity>
...
function StackHeader({navigation, scene, previous}) {
    ...
    return (
        <View style={styles.stackHeaderStyle}>
            ...
            <SwitchOverlay overlayVisible={switchOverlayVisible} animatedHeight={animatedHeight}
                           closeModal={closeSwitchModal} navigation={navigation} alert={logoutAlert} groupNameHandler={changeHeaderHandler}/>
        </View>
    );
}
