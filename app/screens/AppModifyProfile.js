...
function ModifyProfileName({name, setName, prevName}) {
    return (
        <View style={styles.profileModifyNameContainer}>
            <View style={styles.profileModifyNameTextContainer}>
                ...
                <TouchableOpacity style={styles.profileNameModifyBtn}>
                    <Image source={require('@assets/Icons/editPen.png')} style={styles.profileNameModifyBtnIcon}/>
                </TouchableOpacity>
            </View>
...
function SaveBtn({handleChangeProfile}) {
    return (
        <TouchableOpacity style={styles.saveBtn} onPress={handleChangeProfile}>
            <Text style={styles.saveBtnText}>저장</Text>
        </TouchableOpacity>
    )
}

export default function AppModifyProfile({navigation}) {
    ...
    const handleChangeProfile = (navigation) => {
        ...
        call(changeNickname, true, 'PATCH', {nickname: nickname})
            .then(data => {
                if (data.code === 200) {
    ...
    return (
        <View style={{flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: theme.color.white}}>
            <ModifyProfileImage/>
            <ModifyProfileName name={nickname} setName={setNickname} prevName={prevName}/>
            <SaveBtn handleChangeProfile={() => handleChangeProfile(navigation)}/>
        </View>
    );
}
