...
const Join4 = ({route}) => {
    ...
    const handleSignup = async () => {
        if (isFormComplete && isValidNickname && isValidPassword && arePasswordsSame) {
            ...
            await call(signUpApi, false, 'POST', signUpRequest)
                .then(data => {
                    if (data.code === 200) {
            ...
            await Keychain.setInternetCredentials("Wallet", wallet.address, wallet.privateKey);
            await navigation.navigate('Start', {nickname: nickname});
        } else {
